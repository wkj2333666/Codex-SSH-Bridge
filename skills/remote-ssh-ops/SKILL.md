...
- `remote_apply_patch`: `{host, patch}`; accepts one native Codex `*** Begin Patch` envelope with Add, Update, Delete, and `*** Move to:` operations. Sources and destinations must use absolute paths. Do not add a cwd or format field.
