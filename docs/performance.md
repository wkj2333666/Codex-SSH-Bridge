...
MCP dispatch and remote execution are separate measurements. The bridge does not impose a task window or global/per-host runner slots; concurrent request IDs for the same alias share one multiplexed persistent host session. Per-host initialization also covers the first session connection, so a cold burst does not create redundant transports. Output-sink backpressure holds only request-local state, and frame buffers are transferred into bounded forwarding chunks without copying their payload bytes. Both sides scan protocol headers from their existing buffered reader rather than polling or locking the input once per header byte. Warm latency measurements should report MCP dispatch, persistent-session transport, and remote command time separately.
